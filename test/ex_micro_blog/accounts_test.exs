defmodule ExMicroBlog.AccountsTest do
  ...
  describe "users" do
    ...
    test "list_users/0 returns all users when id is nil" do
      user = user_fixture()
      assert Accounts.list_users() == [user]
    end

    test "list_users/1 returns all users when id is valid and sets current_user_following_id" do
      user =
        insert(:user)
        |> Map.put(:password, nil)
        |> Map.put(:current_user_following_id, nil)

      user1 =
        insert(:user1)
        |> Map.put(:password, nil)
        |> Map.put(:current_user_following_id, user.id)

      {:ok, _follower1} =
        Accounts.create_follower(%{"user_id" => user1.id, "follower_id" => user.id})

      assert Accounts.list_users(user.id) == [user1, user]
    end
    ...
    test "get_user_by_username!/1 returns the user with given username" do
      user = user_fixture()
      assert Accounts.get_user_by_username("someusername") == user
    end

    test "list_following!/2 returns the following users with given user_id" do
      user = insert(:user)
      ...
      assert Accounts.list_following(user.id, user.id) == [
  ...
  describe "followers" do
    ...
    test "list_followers/2 returns all followers by user_id" do
      user = insert(:user)
      user1 = insert(:user1) |> Map.put(:password, nil)
      user2 = insert(:user2) |> Map.put(:password, nil)
      ...
      followers = Accounts.list_followers(user.id, user.id)
    ...
    test "delete_follower/1 deletes the follower" do
      ...
      {:ok, follower} = Accounts.create_follower(map)
