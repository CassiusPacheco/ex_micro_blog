defmodule ExMicroBlogWeb.FollowingLive.Index do
  ...
  defp assign_value(user_id, current_user_id, socket) do
    following = Accounts.list_following(user_id, current_user_id)
    user = Accounts.get_user!(user_id)

    case current_user_id do
      nil ->
        assign(socket, current_user: nil, user: user, following: following)

      _ ->
        current_user = Accounts.get_user!(current_user_id)
        assign(socket, current_user: current_user, user: user, following: following)
    end
  end
