defmodule ExMicroBlog.Accounts do
  ...
  set if the `user_id` passed in is valid.
  ...
  """
  def list_users(user_id) when is_nil(user_id) do
    Repo.all(User)
  end
  ...
  """
  def list_users(user_id) do
    # Not the best query. Merges following users with the
    # rest of the users
    following = list_following(user_id)
    following_ids = following |> Enum.map(& &1.id)

    from(u in User,
      where: u.id not in ^following_ids,
      select: %{u | current_user_following_id: 0},
      order_by: [desc: u.inserted_at, desc: u.id]
    )
    |> Repo.all()
    |> Enum.concat(following)
  ...
  def list_followers(user_id) do
    from(u in User,
      ...
      on: f2.user_id == u.id,
      where:
        f1.user_id == ^user_id and
          (f2.follower_id == ^user_id or is_nil(f2.follower_id)),
  ...
  def authenticate_user_by_username(username, password) do
    case get_user_by_username(username |> String.downcase()) do
      %User{} = user ->
